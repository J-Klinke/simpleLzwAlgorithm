...
public class Decoder {
    ...
    public String decode(String[] inputStringAsArray) {
        ...
        for (String string: inputStringAsArray) {
            ...
            if (altCode != -1) {
                prefix.append(dictionary.getEntries().get(altCode)); // prefix is the decoded char/string of altcode
            }

            if (dictionary.getEntries().size() >= code + 1) { // while the size of entries is bigger than or equal to code, the
                                                    // symbol represented by code is in the dictionary
                char sign = dictionary.getEntries().get(code).charAt(0); // in this case sign is the first char of the String
                                                                    // which is represented by code
                String newEntry = prefix.toString() + sign;
                if (!(dictionary.isInDictionary(newEntry))) {
                    dictionary.add(newEntry);
                }
                output.append(dictionary.getEntries().get(code));
            } else if (altCode != -1) {
                char sign = dictionary.getEntries().get(altCode).charAt(0); // in this case sign is the first char of the String
                                                                       // which is represented by altcode
                StringBuilder newEntry = new StringBuilder();
                newEntry.append(prefix).append(sign);
                dictionary.add(newEntry.toString());
                output.append(dictionary.getEntries().get(dictionary.getEntries().indexOf(newEntry.toString())));
            }
    ...
    /**
     * the 'main' method of lzw.Decoder class.
     * @param input the encoded String
     * @return the decoded String
     */
